...
public class PaymentServiceImpl implements PaymentService {
	...
	public PaymentResponse createPayment(PaymentRequest request) {
		...
		PaymentEntity payment = PaymentEntity.builder().paymentReference(UUID.randomUUID())
				.merchantReference(merchant.getMerchantReference()).amount(request.getAmount())
				.currency(request.getCurrency()).paymentMethod(request.getPaymentMethod())
	...
	producer.publishPaymentCreatedEvent(event);
		...
		return PaymentResponse.builder().paymentReference(paymentEntity.getPaymentReference())
				...
				.createdAt(payment.getCreatedAt()).status(paymentEntity.getStatus())
				.merchantReference(paymentEntity.getMerchantReference()).createdAt(paymentEntity.getCreatedAt())
				.build();
	...
	public PaymentResponse getPayment(UUID paymentReference) {
		...
		PaymentEntity payment = repo.findByPaymentReference(paymentReference)
				.orElseThrow(() -> new PaymentNotFoundException("Resource doesn't exists" + paymentReference));
		return PaymentResponse.builder().paymentReference(payment.getPaymentReference())
				.merchantReference(payment.getMerchantReference()).amount(payment.getAmount())
				.currency(payment.getCurrency()).paymentMethod(payment.getPaymentMethod()).status(payment.getStatus())
	...
	public PaymentResponse updatePaymentStatus(UUID paymentReference, UpdatePaymentStatusRequest request) {
		// TODO Auto-generated method stub
		PaymentEntity payment = repo.findByPaymentReference(paymentReference)
				.orElseThrow(() -> new PaymentNotFoundException("Resource doesn't exists"));

		if (!validator.isTransitionAllowed(payment.getStatus(), request.getStatus())) {
			throw new InvalidPaymentStatusTransitionException(
					"Cannot transition from " + payment.getStatus() + "to " + request.getStatus());
		...
		return PaymentResponse.builder().paymentReference(updatePayment.getPaymentReference())
				.merchantReference(updatePayment.getMerchantReference()).amount(updatePayment.getAmount())
				.currency(updatePayment.getCurrency()).status(updatePayment.getStatus())
				.createdAt(updatePayment.getCreatedAt()).build();
	...
	public RefundResponse RefundPayments(UUID paymentRefernce) {
		...
		PaymentEntity payment = repo.findByPaymentReference(paymentRefernce)
				.orElseThrow(() -> new PaymentNotFoundException("Resource doesn;t exists"));
		...
		return RefundResponse.builder().refundReference(savedRefund.getRefundReference())
				.paymentReference(savedRefund.getPaymentReference()).refundAmount(savedRefund.getAmount())
				.status(savedRefund.getStatus()).createdAt(savedRefund.getCreatedAt()).build();

	}

	@Override
	@Transactional
	public RefundResponse partialRefund(UUID paymentReference, PartialRefundRequest request) {
		// TODO Auto-generated method stub
		PaymentEntity payment = repo.findByPaymentReference(paymentReference)
		...
		BigDecimal totalRefundAmount = refundRepo.getTotalRefundAmount(paymentReference);

		BigDecimal remainingAmount = payment.getAmount().subtract(totalRefundAmount);
		if (request.getAmount().compareTo(remainingAmount) > 0) {
			throw new InvalidRefundException("Refund amount exceeds remaining refundable amount.");
		...
		BigDecimal newTotalRefunded = totalRefundAmount.add(request.getAmount());
